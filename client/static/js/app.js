...
class CyberJianghu {
    constructor() {
        ...
        this.currentStoryId = 'demo_story'; // Store the current story ID
        this.storyCreated = false; // Prevent duplicate story creation
    ...
    switchMode(mode) {
        ...
        // Show/hide forms - demo mode shows only story options, no input form
        if (mode === 'demo') {
    ...
    async startDemoMode() {
        ...
        // Only create story once
        if (this.storyCreated) {
            this.log('故事已存在，跳过创建', 'info');
            return;
        }
        ...
        try {
            ...
            this.storyCreated = true; // Mark story as created
